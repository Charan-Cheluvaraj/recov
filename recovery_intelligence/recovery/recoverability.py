...
def determine_recovery_status(
    ...
    Status mapping:
        ...
        - FULLY_RECONSTRUCTED: no internal gaps, parser validation succeeded
        - STRUCTURALLY_VALID_PARTIAL: internal gaps exist, parser validation succeeded
        - PARTIALLY_RECONSTRUCTED: internal gaps exist, parser validation failed
        - STRUCTURALLY_INVALID: no internal gaps, parser validation failed
    """
    st_lower = (status or "").lower()
    if st_lower == "cluster_only" or fragment_count <= 0 or recovered_bytes <= 0:
...
def generate_recovery_reason(
    ...
    """
    if recovery_status == RecoveryStatus.UNRECOVERABLE.value or recovered_bytes == 0:
        return (
...
def write_recovered_artifact(
    ...
    """
    Write candidate bytes to disk at recovered/recovered_<candidate_id>.<ext>
...
def assess_recoverability(
    ...
    1. Computes recovered bytes and missing/unknown gap bytes.
    2. Writes the recovered candidate artifact to recovered/recovered_<candidate_id>.<ext>.
    ...
    for fid in reconstructed_file.fragment_ids:
        if fid in frag_map:
            ...
            recovered_bytes += len(chunk) if chunk else frag.length

    # If raw_chunks is empty but candidate file was already generated in Stage 5:
        ...
        if stage5_path.exists():
            with open(stage5_path, "rb") as sf:
                ...
                if recovered_bytes == 0:
                    recovered_bytes = len(c_bytes)

    assembled_candidate_bytes = b"".join(raw_chunks)
    if recovered_bytes == 0 and assembled_candidate_bytes:
    ...
    # 3. Calculate recoverability metrics
    ...
    # 6. Generate factual machine-generated explanation
    ...
    # 7. Update ReconstructedFile model
